class Bon_Sortie {
  ...
  Bon_Sortie(
      this.bon_sortie_id, this.motif, DateTime date_sortie, this.utilisateur_id)
      : date_sortie =
            DateTime(date_sortie.year, date_sortie.month, date_sortie.day);
  List<Bon_Sortie> bonsSortie = [];

  // Création d'un bon de sortie
  void creerBonSortie(int bon_sortie_id, String motif, DateTime date_sortie,
      int utilisateur_id) {
    var nouveauBonSortie =
        Bon_Sortie(bon_sortie_id, motif, date_sortie, utilisateur_id);
  ...
  void afficherBonsSortie() {
    ...
    for (var bon in bonsSortie) {
      ...
      String dateAffichee = bon.date_sortie != null
          ? "${bon.date_sortie!.year}-${bon.date_sortie!.month}-${bon.date_sortie!.day}"
          : "Date non définie";
      print(
          "ID: ${bon.bon_sortie_id}, Motif: ${bon.motif}, Date: $dateAffichee, Utilisateur ID: ${bon.utilisateur_id}");
    }
    print("\n");
  }

  // Modification d'un bon de sortie
  void modifierBonSortie(int index, int bon_sortie_id, String motif,
      DateTime date_sortie, int utilisateur_id) {
  ...
  Bon_Sortie? rechercherBonSortieParId(int bon_sortie_id) {
    var bon = bonsSortie.firstWhere(
      ...
      orElse: () {
        ...
        return Bon_Sortie(
            0, '', DateTime.now(), 0); // Ou une autre valeur par défaut
      },
    );
    return bon;
  }
  ...
  List<Bon_Sortie> rechercherBonsSortieParDate(DateTime date) {
    var bons = bonsSortie.where((bon) =>
        ...
        bon.date_sortie!.year == date.year &&
        bon.date_sortie!.month == date.month &&
        bon.date_sortie!.day == date.day);
    return bons.toList();
  }
}

// void main() {
//    var gestionnaire =  Bon_Sortie(0, '', null, 0); // Fournir les valeurs par défaut pour les arguments requis
